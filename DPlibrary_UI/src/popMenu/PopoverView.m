...
+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withStringArray:(NSArray *)stringArray withIndex:(NSUInteger)_index delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    [popoverView showAtPoint:point inView:view withTitle:title withStringArray:stringArray withIndex:_index];
    popoverView.delegate = delegate;
    [popoverView release];
    return popoverView;
}
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withText:(NSString *)text {
    ...
    UILabel *textView = [[[UILabel alloc] initWithFrame:CGRectMake(0, 0, textSize.width, textSize.height)] autorelease];
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withStringArray:(NSArray *)stringArray withIndex:(NSUInteger) _index {
    NSMutableArray *labelArray = [[[NSMutableArray alloc] initWithCapacity:stringArray.count] autorelease];
    ...
    for(int i = 0; i < stringArray.count; i++)
    {
        ...
        UIView *showView = [[[UIView alloc] initWithFrame:CGRectMake(0, 0, 130, 30)] autorelease];
        if(i == _index)
        {
            UIImage *iconImage = [UIImage imageNamed:@"iconyes2.png"];
            UIImageView *iconImageView = [[[UIImageView alloc] initWithFrame:CGRectMake(2, 4, 25, 25)] autorelease];
            [iconImageView setCenterY:showView.centerY];
            [iconImageView setImage:iconImage];
            [showView addSubview:iconImageView];
        }


        UILabel *label = [[[UILabel alloc] initWithFrame:CGRectMake(12, 0, textSize.width, textSize.height)] autorelease];
        label.backgroundColor = [UIColor clearColor];
        label.font = font;
        label.textAlignment = kTextAlignment;
        label.textColor = kTextColor;
        label.text = string;
        label.layer.cornerRadius = 4.f;
        [label setCenterY:showView.centerY];
        [showView addSubview:label];
        [labelArray addObject:showView];
    }
...
- (void)showSuccess {
    UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"iconyes2.png"]];
    imageView.alpha = 0.f;
    imageView.frame = CGRectMake(CGRectGetMidX(contentView.bounds) - 20.f, CGRectGetMidY(contentView.bounds) - 20.f , 40.f, 40.f);
    imageView.transform = CGAffineTransformMakeScale(0.1f, 0.1f);
